...
async def enrich_ticker_data(ticker: str, gapper_data: Dict[str, Any], exchange_id: str) -> Event:
    """
    Enriches a ticker with additional data.
    ...
    return Event(
        author="enrich_ticker_data",
        content=types.Content(
            parts=[types.Part(text=enriched_data.model_dump_json())]
        ),
    )
